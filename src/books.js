function findAuthorById(authors, id) {//returns author object with matching ID
  const found = authors.find(author => author.id == id)
  return found;
}

function findBookById(books, id) {//returns book object with matching ID
  let bookObject = books.find(book => book.id == id)
  return bookObject;
}

function partitionBooksByBorrowedStatus(books) {//we need to return two arrays in a array!!
